...
| `API rate limit already exceeded` | Too many GraphQL requests in a short window | Wait for the reset window: `date -r $(gh api rate_limit --jq '.resources.graphql.reset') "+resets at %H:%M:%S"` |
