...
- Posibles conflictos en workdir si dos sesiones escriben al mismo item: el path per-item (`.../<externalId>/`) aísla **items distintos**, no escritores concurrentes sobre el **mismo** item. Mitigaciones previstas: (a) el orchestrator evita dos dispatches activos del mismo specialist en el mismo item; (b) specialists distintos en el mismo item usarán subdirs por rol bajo `<externalId>/` (diseño en Sesión 10-11); (c) colisiones no previstas quedan como riesgo conocido hasta definir locking o reconciliación.
